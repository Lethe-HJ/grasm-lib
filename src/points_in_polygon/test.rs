...
mod tests {
    ...
    fn test_square_with_hole() {
        ...
        // Test outer area
        assert_eq!(point_in_polygon(&[4.0, 1.5], &polygon, &rings, true), vec![0]);
        assert_eq!(point_in_polygon(&[-1.0, 1.5], &polygon, &rings, true), vec![0]);
        
        // Test hole area
        assert_eq!(point_in_polygon(&[1.5, 1.5], &polygon, &rings, true), vec![0]);
        
        // Test valid area
        assert_eq!(point_in_polygon(&[0.5, 0.5], &polygon, &rings, true), vec![1]);
        assert_eq!(point_in_polygon(&[2.5, 0.5], &polygon, &rings, true), vec![1]);
        
        // Test boundary
        assert_eq!(point_in_polygon(&[3.0, 1.5], &polygon, &rings, true), vec![1]);
        assert_eq!(point_in_polygon(&[3.0, 1.5], &polygon, &rings, false), vec![0]);
    }
    ...
    fn test_square_with_hole_grid_points() {
        ...
        println!("Testing with {} x {} = {} grid points (step size: {})", 
                 points_per_axis, points_per_axis, num_points, step);
        ...
        println!("Point in polygon test with {} points took: {:?}", num_points, duration);
        ...
        println!("Accuracy: {}/{} = {:.6}%", correct_count, total_count, accuracy);
    ...
    fn test_circle_with_holes_grid_points() {
        ...
        println!("Testing circle with two holes: {} x {} = {} grid points (step size: {})", 
                 points_per_axis, points_per_axis, num_points, step);
        ...
        for i in 0..num_points {
            ...
            let expected = if dist_to_main > 5.0 { // 在大圆外
                0
            } else if dist_to_hole1 < 1.0 || dist_to_hole2 < 1.0 { // 在任一小圆内
                0
            } else {
                1
            };
